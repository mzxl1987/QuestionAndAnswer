 ...
 *
 * @Bean注解的方法会被Spring容器所加载&生成bean定义,运行的时候服务会请求这些bean
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 *
 * 可以通过AnnotationConfigApplicationContext | AnnotationConfigWebApplicationContext来生成@Configuration配置的类
 *
 * <pre class="code">
 ...
 *
 * 上述例子, <context:annotation-config/>标签是必备的,为了启用 ConfigurationClassPostProcessor & 其他关联的注解更方便的操作@Configuration类
 *
 ...
 * 
 * @Configuration 与元注解 @Component相关联, @Configuration注解的类将被当作组建候选扫描(典型的使用<context:component-scan/>)
 * 所以高级的使用@Autowired / @Inject, 常规的使用@Component,如果是单构造函数,则将被显示调用
 *   
 *
 * <pre class="code">
 * &#064;Configuration
 ...
 * @Configuration类不仅可以使用组件扫描,开可以使用@CompomentScan进行组建扫描
